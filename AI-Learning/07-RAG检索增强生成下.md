...
> **严谨定义**：混合检索结合了语义检索（向量相似度）与关键词检索（BM25/全文索引），通过融合策略（如 RRF、加权分数归一化）合并两路结果。语义检索擅长捕捉意图和同义词，关键词检索擅长精确匹配专有名词和缩写。两者互补可显著提升召回率和准确率。

> **通俗理解**：就像找医生看病——语义检索是“描述症状找相似病例”（模糊匹配），关键词检索是“搜病名找精确结果”（精确匹配）。两种方式结合起来，既不会漏掉相似病例，也不会找错病名。双管齐下比单一方式靠谱得多。
...
> **严谨定义**：重排序（Reranking）是在初始检索（召回）后，使用更精细的交叉编码器（Cross-Encoder）模型对候选文档进行二次打分的过程。与双塔模型（Bi-Encoder）不同，Cross-Encoder 将查询和文档拼接后联合编码，能捕捉更细粒度的交互特征，因此相关性判断更准确。典型流程为“Bi-Encoder 召回 Top-20 → Cross-Encoder 精排 → 取 Top-5”。

> **通俗理解**：就像招聘流程——向量检索是“HR 快速筛简历”（看关键词，快但粗），重排序是“面试官仔细看简历”（逐份评估，慢但准）。先用快速方法缩小范围，再用精细方法挑出最好的。两步结合，又快又准。

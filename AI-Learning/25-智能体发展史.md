...
> **严谨定义**：符号主义是 AI 研究的早期范式，核心假设是“智能 = 符号操作”。通过将问题表示为符号结构和规则，使用逻辑推理（如状态空间搜索、产生式规则）实现智能行为。代表成果包括 Logic Theorist（1956）、GPS（1959）、专家系统 MYCIN（1976）。局限是知识获取瓶颈和无法处理不确定性。

> **通俗理解**：就像你写 if-else 语句——“如果分数>80，就推荐面试”。符号主义就是让 AI 按你写的规则办事。规则写得好，AI 就“聪明”；规则写不全，AI 就“笨”。问题是世界太复杂，规则写不完。

### 专家系统（Expert System）

> **严谨定义**：专家系统是基于领域专家知识构建的规则推理系统，包含知识库（存储专家规则）、推理引擎（应用规则进行链式推理）和解释器（说明推理过程）。在特定领域（如医疗诊断、化学分析）可达到甚至超越专家水平，但维护成本高、知识获取困难、无法处理规则外的情况。

> **通俗理解**：就像一个“老专家经验库”——把医生的诊断经验整理成 500 条规则，输入症状后按规则推理出诊断。MYCIN 诊断准确率 69%，超过普通医生 55%。但问题是：规则太多难维护，新疾病没规则就束手无策。

### 强化学习（Reinforcement Learning）

> **严谨定义**：强化学习是一种通过试错学习的机器学习范式，Agent 在环境中执行动作，接收奖励或惩罚信号，目标是最大化累积奖励。核心概念包括状态（State）、动作（Action）、奖励（Reward）、策略（Policy）。代表成果包括 AlphaGo、游戏 AI。局限是需要大量试错、只能处理定义明确的环境。

> **通俗理解**：就像训练小狗——做对了给奖励，做错了不给。小狗通过不断尝试，逐渐学会“坐下”“握手”。强化学习也是这样——AI 在迷宫里乱撞，撞到终点就给奖励，慢慢就学会了最优路径。
...
```mermaid
graph TD
    A[1950s-1970s 符号主义] -->|规则写不完| B[1980s-2000s 联结主义]
    B -->|需要大量标注| C[2000s-2017 统计学习]
    C -->|特征工程瓶颈| D[2017-2022 Transformer]
    D -->|通用语言能力| E[2023-至今 LLM Agent]
    A -->|特点| F[人写规则 机器执行]
    B -->|特点| G[人设计网络 数据训练]
    C -->|特点| H[人设计特征 模型学习]
    D -->|特点| I[自监督学习 迁移学习]
...
| 上一课 | 下一课 | 返回 |
| --- | --- | --- |
| [第 24 课：端到端案例——AI 招聘系统架构全解](24-端到端企业案例AI招聘系统.md) | [第 26 课：Agent 经典范式构建](26-Agent经典范式构建.md) | [课程目录](README.md) |

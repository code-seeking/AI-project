...
### Prompt 工程化（Prompt Engineering as Code）

> **严谨定义**：Prompt 工程化是将 Prompt 视为软件资产的管理方法论，包括版本控制（Version Control）、自动化测试（Automated Testing）、灰度发布（Canary Release）和回滚机制（Rollback）。它要求 Prompt 具备可追溯性（Traceability）、可重复性（Reproducibility）和可度量性（Measurability）。

> **通俗理解**：就像你写 Java 代码不会直接在生产环境改代码一样，Prompt 也不能"随手一改就上线"。你需要像管理代码一样管理 Prompt：改了什么要有记录（Git commit），改了之后要测试（JUnit），上线要灰度（10% 流量），出问题要能回滚（git revert）。

### Prompt 模板系统（Prompt Template System）

> **严谨定义**：Prompt 模板系统是一种将 Prompt 定义与业务逻辑分离的架构设计，通过变量替换（Variable Substitution）、条件渲染（Conditional Rendering）和模板继承（Template Inheritance）实现 Prompt 的动态生成。核心组件包括模板引擎（Template Engine）、变量管理器（Variable Manager）和渲染器（Renderer）。

> **通俗理解**：就像 Thymeleaf 模板一样，Prompt 模板也有"占位符"和"条件判断"。你写一个"简历分析模板"，里面用 `{{resume_text}}` 占位，运行时把真实简历填进去。不同场景（有/无教育背景）自动渲染不同内容，不用写多个 Prompt。

**初学者的误区**：Prompt 只是字符串，写在代码里就行

**生产环境的事实**：Prompt 是会持续演化的"逻辑"

- → 需要**版本管理**（谁改的、改了什么、为什么）
- → 需要**测试**（改了会不会影响其他功能）
- → 需要**灰度**（改完直接全量上线有风险）
- → 需要**回滚**（效果变差立刻恢复）

> **Prompt 管理不善的代价**：某次改了一个 Prompt 的措辞，效果变差，但不知道改了什么，只能靠回忆回滚 → 灾难
...
> 你项目的实践：AI 建议需要"一键执行" → 同样道理，Prompt 版本也要能"一键回滚"
...
**按比例灰度**：10% 请求 → 新版，90% → 旧版（随机取模：`userId % 10 == 0` → 新版）
...
4. **兖底回答**：不知道时统一话术（"建议咨询HR部门"）
5. **结构化兖底**：LLM 输出 JSON 失败时 → 解析修正 → 重试
...
| 上一课 | 下一课 | 返回 |
| --- | --- | --- |
| [第 16 课：企业级 RAG 深度实战](16-企业级RAG深度实战.md) | [第 18 课：LLM 微调实战](18-LLM微调实战.md) | [课程目录](README.md) |
